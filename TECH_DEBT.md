...
## 3. There is no import-boundary test

The four packages (`config`, `corrector`, `logger`, `shell`) are cleanly separated today: `corrector` holds pure matching logic, `shell` holds the PowerShell integration, `config` holds loading and `logger` holds statistics. `main.go` wires them.

Nothing holds that separation. The specific direction worth protecting is that `corrector` must not import `shell` or `config`: the correction engine is pure computation over strings, which is exactly why `engine_test.go` can be 672 lines of straightforward table tests. The moment it reaches for the shell or the filesystem, that testability goes and does not come back.

One import scan asserting `corrector` imports nothing from the other three packages closes it. That is the highest value-per-line item in this file.

## 4. The coverage target has no threshold
...
## 5. The install and uninstall scripts repeat each other's knowledge
...
## 6. The Makefile does not work on the tool's only supported platform
...
Either give `build.ps1` matching `test`, `lint` and `coverage` functions, or replace the Makefile with a PowerShell task script. The second is more honest for a Windows-only tool. Whichever is chosen, items 2, 3 and 4 all add checks that need somewhere to live, and that somewhere has to be runnable on Windows.

---

## Looks like debt, not worth touching

- The flat package layout (`config`, `corrector`, `logger`, `shell`) rather than `internal/{domain,application,infrastructure}`. At 3,300 lines with one clear input and one clear output, four cohesive packages is the proportionate structure and `ARCHITECTURE.md` describes them accurately as modules. Item 3 asks for one import rule, not for four directories.
...
- **`corrector/engine.go` being pure string logic with no I/O.** This is why its tests are simple table tests and why the package is worth protecting. Item 3 exists to keep it that way.
