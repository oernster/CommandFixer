...
This is a small tool (roughly 3,300 lines across eighteen Go files) and the file is short in proportion. `build.ps1` carries the whole workflow (build, test, race, lint, coverage and clean) and `ARCHITECTURE.md` describes the packages honestly as modules rather than claiming a layering the code does not have.
